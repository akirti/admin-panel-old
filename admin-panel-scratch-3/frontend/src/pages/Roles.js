...
import { rolesAPI, permissionsAPI, domainsAPI } from '../services/api';
...
const Roles = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const [rolesRes, permissionsRes, domainsRes] = await Promise.all([
        rolesAPI.list({ search: search || undefined, page: pagination.page, limit: pagination.limit }),
        permissionsAPI.list({ limit: 100 }),
        domainsAPI.list({ limit: 100 })
      ]);
      setRoles(rolesRes.data.data || rolesRes.data);
      setPagination(prev => ({ ...prev, ...(rolesRes.data.pagination || {}) }));
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      await rolesAPI.update(editingItem.roleId, formData);
  ...
  const handleDelete = async (item) => {
    ...
    try {
      await rolesAPI.delete(item.roleId);
