...
const Playboards = () => {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      await playboardsAPI.update(editingItem.id, payload);
  ...
  const handleDelete = async (item) => {
    ...
    try {
      await playboardsAPI.delete(item.id);
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      await playboardsAPI.upload(formData, {
  ...
  const handleDownload = async (item) => {
    try {
      const response = await playboardsAPI.download(item.id);
                    ...
                    <div className="bg-gray-50 rounded p-2 space-y-1">
                      {currentRowAction.filters.map((filter, idx) => (
                        <div key={idx} className="flex items-center justify-between bg-white p-2 rounded border text-sm">
                          ...
                          <button type="button" onClick={() => removeActionFilter(idx)} className="text-red-500 hover:text-red-700">×</button>
                        </div>
                      ))}
                    </div>
                  )}
