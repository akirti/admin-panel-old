...
function UploadForm({ entityType, setEntityType, file, onFileChange, uploading, onUpload, sendPasswordEmails, onTogglePasswordEmails, onDownloadTemplate }) {
  return (
      ...
      <div className="space-y-4">
        ...
        {entityType === 'users' && (
          <div className="flex items-center justify-between p-3 bg-surface-secondary rounded-lg">
            ...
            <button
              type="button"
              onClick={onTogglePasswordEmails}
...
function GcsUploadSection({ gcsStatus, gcsPath, onGcsPathChange, uploading }) {
  return (
        ...
        <div className="space-y-3">
          <div>
            ...
            <input
              ...
              value={gcsPath}
              onChange={(e) => onGcsPathChange(e.target.value)}
            />
          </div>
          <button
...
/* ─── Helper: show upload result toasts ─── */
function showUploadResultToasts(data) {
  if (data.successful > 0) {
    toast.success(`Successfully processed ${data.successful} records`);
  }
  if (data.failed > 0) {
    toast.error(`Failed to process ${data.failed} records`);
  }
}

/* ─── Templates & Instructions Section ─── */
function TemplatesSection({ onDownloadTemplate, gcsStatus, gcsPath, onGcsPathChange, uploading }) {
  return (
    <div className="card p-6">
      <h3 className="text-lg font-semibold text-content mb-4 flex items-center gap-2">
      ...
      <div className="flex space-x-3">
        <button onClick={() => onDownloadTemplate('xlsx')} className="flex items-center gap-2 px-4 py-2 border border-edge rounded-md text-content-secondary hover:bg-surface-hover">
          <FileSpreadsheet size={16} />Excel Template
        </button>
        <button onClick={() => onDownloadTemplate('csv')} className="flex items-center gap-2 px-4 py-2 border border-edge rounded-md text-content-secondary hover:bg-surface-hover">
          <Download size={16} />CSV Template
        </button>
      </div>
      <GcsUploadSection gcsStatus={gcsStatus} gcsPath={gcsPath} onGcsPathChange={onGcsPathChange} uploading={uploading} />
    </div>
  );
}
...
const BulkUploadPage = () => {
  ...
  useEffect(() => {
    bulkAPI.getGCSStatus()
      .then((response) => setGcsStatus(response.data))
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      showUploadResultToasts(response.data);
    } catch (error) {
  ...
  const handleDownloadTemplate = async (format) => {
    try {
      ...
      downloadBlob(response.data, `${entityType}_template.${format}`);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <UploadForm
          entityType={entityType} setEntityType={handleEntityTypeChange}
          file={file} onFileChange={handleFileChange}
          uploading={uploading} onUpload={handleUpload}
          sendPasswordEmails={sendPasswordEmails}
          onTogglePasswordEmails={() => setSendPasswordEmails(!sendPasswordEmails)}
          onDownloadTemplate={handleDownloadTemplate}
        />
        <TemplatesSection
          onDownloadTemplate={handleDownloadTemplate}
          gcsStatus={gcsStatus} gcsPath={gcsPath}
          onGcsPathChange={setGcsPath} uploading={uploading}
        />
      </div>
