  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-neutral-500 mt-1">Overview of your admin panel</p>
      </div>
        ...
        <Link to={isSuperAdmin() ? '/admin/roles' : '#'} className="card p-4 hover:shadow-md transition-all">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-neutral-900">{stats?.total_roles || 0}</p>
            </div>
        ...
        <Link to={isSuperAdmin() ? '/admin/groups' : '#'} className="card p-4 hover:shadow-md transition-all">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-neutral-900">{stats?.total_groups || 0}</p>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Link to={isSuperAdmin() ? '/admin/domains' : '/management/domains'} className="card p-4 hover:shadow-md transition-all">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-neutral-500">Domains</p>
              <p className="text-2xl font-bold text-neutral-900">{stats?.total_domains || 0}</p>
            </div>
        ...
        <Link to={isSuperAdmin() ? '/admin/scenarios' : '#'} className="card p-4 hover:shadow-md transition-all">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-neutral-500">Scenarios</p>
              <p className="text-2xl font-bold text-neutral-900">{stats?.total_scenarios || 0}</p>
            </div>
        ...
        <Link to={isSuperAdmin() ? '/admin/playboards' : '#'} className="card p-4 hover:shadow-md transition-all">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-neutral-500">Playboards</p>
              <p className="text-2xl font-bold text-neutral-900">{stats?.total_playboards || 0}</p>
            </div>
        ...
        <Link to={isSuperAdmin() ? '/admin/configurations' : '#'} className="card p-4 hover:shadow-md transition-all">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-neutral-500">Configurations</p>
              <p className="text-2xl font-bold text-neutral-900">{stats?.total_configurations || 0}</p>
            </div>
      ...
      {/* Request Stats Row */}
      <div>
        <h4 className="text-l font-bold text-neutral-900">
          Scenario Request Statistics
        </h4>
        <p className="text-neutral-500 mt-1">Overview of reaquested scenarios</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Link to="/admin/feedback" className="card p-4 hover:shadow-md transition-all">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-neutral-900">{feedbackStats.total_feedback}</p>
            </div>
          ...
          <div className="flex gap-0.5 mt-2">
            {[1, 2, 3, 4, 5].map((star) => (
              <Star
                key={star}
                size={14}
                className={star <= Math.round(feedbackStats.avg_rating) ? 'fill-amber-400 text-amber-400' : 'text-neutral-300'}
              />
            ))}
          </div>
        </div>
        ...
        <div className="card p-4">
          <div className="flex items-center justify-between mb-2">
            <p className="text-sm text-neutral-500">Rating Distribution</p>
          </div>
          <div className="flex gap-1">
            {Object.entries(feedbackStats.rating_distribution).map(([rating, count]) => (
              <div key={rating} className="flex-1 text-center">
                ...
                <div className="text-sm font-semibold text-neutral-700">{count}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {analytics.activity_trend && analytics.activity_trend.length > 0 && (
            <div className="card p-6">
              <h3 className="text-lg font-semibold text-neutral-900 mb-4">Activity Trend (Last 7 Days)</h3>
                ...
                {analytics.activity_trend.map((item) => {
                  ...
                  return (
                    <div key={item.date} className="flex-1 flex flex-col items-center">
                      <div className="text-xs font-medium text-neutral-900 mb-1">{item.count}</div>
                      <div
          ...
          {analytics.role_distribution && analytics.role_distribution.length > 0 && (
            <div className="card p-6">
              <h3 className="text-lg font-semibold text-neutral-900 mb-4">Role Distribution</h3>
                  ...
                  return (
                    <div key={role.role}>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-sm text-neutral-700 truncate">{role.role}</span>
                        <span className="text-sm text-neutral-500">{role.count} ({percentage}%)</span>
                      </div>
      ...
      {analytics && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Top Active Users */}
          <div className="card p-6">
            <h3 className="text-lg font-semibold text-neutral-900 mb-4">Top Active Users (Last 7 Days)</h3>
            <div className="space-y-3">
              {(!analytics.top_active_users || analytics.top_active_users.length === 0) ? (
                <p className="text-neutral-500 text-center py-4">No activity data available</p>
              ) : (
                analytics.top_active_users.map((user, index) => (
                  <div key={user.user_email} className="flex items-center justify-between py-2 border-b border-neutral-100 last:border-0">
                    <div className="flex items-center">
                      ...
                      <div>
                        <p className="text-sm font-medium text-neutral-900 truncate max-w-[180px]">{user.user_email}</p>
                      </div>
                    </div>
          ...
          {/* Recent User Signups */}
          <div className="card p-6">
            <h3 className="text-lg font-semibold text-neutral-900 mb-4">Recent User Signups</h3>
            <div className="space-y-3">
              {(!analytics.recent_signups || analytics.recent_signups.length === 0) ? (
                <p className="text-neutral-500 text-center py-4">No recent signups</p>
              ) : (
                analytics.recent_signups.map((signup, index) => (
                  <div key={index} className="flex items-center justify-between py-2 border-b border-neutral-100 last:border-0">
                    <div className="flex items-center">
                      ...
                      <div>
                        <p className="text-sm font-medium text-neutral-900">{signup.full_name || 'N/A'}</p>
      ...
      {summary?.configurations && (
        <div className="card p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-neutral-900">Configurations Overview</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Status Summary */}
        <div className="card p-6">
          <h3 className="text-lg font-semibold text-neutral-900 mb-4">Entity Status Summary</h3>
          <div className="space-y-4">
            {summary && Object.entries(summary)
              .filter(([key]) => !key.includes('module') && !key.includes('configurations'))
              .map(([entity, statusData]) => (
                <div key={entity} className="flex items-center justify-between py-2 border-b border-neutral-100 last:border-0">
                  <span className="text-neutral-600 capitalize">{entity}</span>
        ...
        {/* Recent Logins */}
        <div className="card p-6">
          <h3 className="text-lg font-semibold text-neutral-900 mb-4">Recent Logins</h3>
          <div className="space-y-3">
            {recentLogins.length === 0 ? (
              <p className="text-neutral-500 text-center py-4">No recent logins</p>
            ) : (
              recentLogins.map((login, index) => (
                <div key={index} className="flex items-center justify-between py-2 border-b border-neutral-100 last:border-0">
                  <div className="flex items-center">
                    ...
                    <div>
                      <p className="text-sm font-medium text-neutral-900">{login.full_name}</p>
      ...
      <div className="card p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <Link
            ...
            className="flex flex-col items-center p-4 rounded-lg border border-neutral-200 hover:border-red-300 hover:bg-red-50 transition-colors"
          >
          ...
          >
            ...
            <span className="text-sm font-medium text-neutral-700">Manage Roles</span>
          </Link>
          <Link
          ...
          >
            ...
            <span className="text-sm font-medium text-neutral-700">Permissions</span>
          </Link>
          <Link
          ...
          >
            ...
            <span className="text-sm font-medium text-neutral-700">Configurations</span>
          </Link>
          <Link
          ...
          >
            ...
            <span className="text-sm font-medium text-neutral-700">Bulk Upload</span>
          </Link>
        </div>
      </div>
    </div>
