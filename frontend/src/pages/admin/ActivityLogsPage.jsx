...
const getActionColor = (actionType) => {
  return ACTION_COLORS[actionType] || 'bg-neutral-100 text-neutral-800';
};
...
function StatsCards({ stats, getActionColor }) {
  return (
...
/* ─── Header with exports and days selector ─── */
function ActivityLogsHeader({ days, onDaysChange }) {
  return (
...
/* ─── Filter controls ─── */
function ActivityFiltersBar({ userEmail, onUserEmailChange, entityType, onEntityTypeChange, entityTypes, action, onActionChange, actions }) {
  const inputClass = "w-full px-3 py-2 border border-edge rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-500";
  return (
    <div className="card">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          <label className="block text-sm font-medium text-content-secondary mb-1">User Email</label>
          <input type="text" value={userEmail} onChange={onUserEmailChange} placeholder="Filter by user email..." className={inputClass} />
        </div>
        <div>
          <label className="block text-sm font-medium text-content-secondary mb-1">Entity Type</label>
          <select value={entityType} onChange={onEntityTypeChange} className={inputClass}>
        ...
        <div>
          ...
          <select value={action} onChange={onActionChange} className={inputClass}>
...
const ActivityLogsPage = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [logsRes, statsRes, actionsRes, entityTypesRes] = await Promise.all([
        activityLogsAPI.list({ page: pagination.page, limit: pagination.limit, user_email: userEmail || undefined, entity_type: entityType || undefined, action: action || undefined, days: days || undefined }),
  ...
  const handleDaysChange = (e) => { setDays(parseInt(e.target.value)); resetPage(); };
  const handleUserEmailChange = (e) => { setUserEmail(e.target.value); resetPage(); };
  const handleEntityTypeChange = (e) => { setEntityType(e.target.value); resetPage(); };
  const handleActionChange = (e) => { setAction(e.target.value); resetPage(); };

  const showTimeline = stats?.timeline && stats.timeline.length > 0;

  return (
    <div className="space-y-6">
      <ActivityLogsHeader days={days} onDaysChange={handleDaysChange} />
      {stats && <StatsCards stats={stats} getActionColor={getActionColor} />}
      <ActivityFiltersBar
        userEmail={userEmail} onUserEmailChange={handleUserEmailChange}
        entityType={entityType} onEntityTypeChange={handleEntityTypeChange} entityTypes={entityTypes}
        action={action} onActionChange={handleActionChange} actions={actions}
      />
