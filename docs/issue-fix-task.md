...
### [x] FIX-C2: Add ownership check to scenario request update
...
### [x] FIX-C3: Add role hierarchy validation to PUT /users/{user_id}
...
### [x] FIX-C4: Narrow CSRF exemption to only login/register/refresh
...
### [x] FIX-C5: Fix token validation timestamp logic
...
### [x] FIX-C6: Sanitize HTML in RequestDetailPage
...
### [x] FIX-C7: Restrict user deletion to require_super_admin
- **File**: `backend/src/easylifeauth/api/users_routes.py:426-463`
- **Action**: Change `require_group_admin` to `require_super_admin` for DELETE endpoint. Group-editors/group-admins should not delete users.
- **Status**: FIXED
...
### [x] FIX-H1: Add require_admin_or_editor to Jira write endpoints
...
### [x] FIX-H2: Restrict API config read endpoints to super-admin
- **File**: `backend/src/easylifeauth/api/api_config_routes.py:31-117`
- **Action**: Change `get_current_user` to `require_super_admin` on all GET endpoints (list, count, tags, get by id, get by key).
- **Status**: FIXED

### [x] FIX-H3: Align user delete to require_admin on both endpoints
- **File**: `backend/src/easylifeauth/api/users_routes.py:426` (covered by FIX-C7)
- **Status**: FIXED (merged with FIX-C7)
...
### [x] FIX-H5: Add ownership check to scenario request file operations
...
- **Status**: FIXED (merged with FIX-C2)

### [x] FIX-H6: Set explicit CORS origins
...
### [x] FIX-H7: Escape regex in all search parameters
...
### [x] FIX-H8: Remove temp password from API response
...
### [x] FIX-H9: Fix resolve_domains field name
...
### [x] FIX-H10: Fix token refresh to resolve domains from groups/roles
...
- **Status**: SKIPPED (playboard queries domain_scenarios collection which uses domainKey - not a bug)

### [x] FIX-H12: Fix register to resolve domains from assigned groups
...
### [x] FIX-M2: Restrict distribution list read to require_group_admin
...
### [x] FIX-M6: Restrict password reset to require_admin
...
### [x] FIX-M8: Clean user.domains on domain deletion
...
### [x] FIX-M9: Fix "All Domains" badge for empty domains
...
### [x] FIX-M10: Fix hardcoded /admin/ path in ScenarioRequestsManagement
...
### [x] FIX-L3: Add auth to health metrics endpoint
...
### [x] FIX-L6: Delete all reset tokens for user after successful reset
